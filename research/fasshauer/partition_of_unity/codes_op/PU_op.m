%-------------------------------------------------------------------------%
%
% File: PU_op(M,dsites,neval,npu,rbf,wf,f,rhs,h,param);
%
...
%
%-------------------------------------------------------------------------%
function [epoints, Pf, radii, epsilon] = PU_op(M, dsites, neval, npu, rbf, f, rhs, h, param, puctrs)
...
%     break
    ...
    eidx = IntegerBased_MD_RangeSearchAniso(puctrs(j, :), radii(j, :), edxx, edx);
    elocpts(j).ind = eidx;
end
