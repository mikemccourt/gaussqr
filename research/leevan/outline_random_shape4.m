...
for k=1:length(stuff)
    ...
    legend([hsol, hfill], {sprintf('$\\varepsilon=%3.2f$', ep), 'random, $\tau=\sqrt{10}$'}, 'location', 'best', 'interpreter', 'latex', 'fontsize', fontsize)
    hold off
end

savefig('examples_1d_svd')
saveas(gcf, 'examples_1d_svd', 'png')