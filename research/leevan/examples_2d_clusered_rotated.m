% Allow for using rotated kernels in interpolation of clusetered data
...
yf = @(x) ( ...
    1.3 * exp(-3 * (x(:, 1) .^ 2 + x(:, 2) .^ 2)) + ...
    1.2 * exp(-4 * (.7 * (x(:, 1) - .5) .^ 2 + 1.3 * (x(:, 1) - .5) .^ 2)) + ...
    .5 * exp(-2 * sqrt(.8 * (x(:, 1) - .25) .^ 2 + .7 * (x(:, 1) - .25) .^ 2))...
);
...
for N=Nvec
    ...
    for tcount=1:num_runs
        ...
        for v=varvec
            a = (ep * 3)^2 / v;
            b = v / (ep * 3);
            epvec_tight = gamrnd(a, b, floor(2 * N / 3), 2);
            
            a = ep^2 / v;
            b = v / ep;
            epvec_full = gamrnd(a, b, N - length(epvec_tight), 2);
            
            epvec = [epvec_tight; epvec_full];
...
filename = 'examples_2d_rotated_clustered_2';
savefig(filename)
