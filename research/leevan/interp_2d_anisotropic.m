% Allow for using anisotropic kernels in interpolation
...
for N=Nvec
    ...
    for tcount=1:num_runs
        ...
        for v=varvec
            ...
            DM2 = bsxfun(@times, x(:, 1) - x(:, 1)', epvec(:, 1)').^2 + ...
                bsxfun(@times, x(:, 2) - x(:, 2)', epvec(:, 2)').^2;
            DMeval2 = bsxfun(@times, xeval(:, 1) - x(:, 1)', epvec(:, 1)').^2 + ...
                bsxfun(@times, xeval(:, 2) - x(:, 2)', epvec(:, 2)').^2;
