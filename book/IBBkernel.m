...
%          (2*ep^3*(sinh(2*ep*L)+cosh(2*ep*L)-1).^2);
    ...
    % GF: It looks like this differs from the series by some constant multiple
...
%          exp(2*ep*max(x,z)).*(ep*min(x,z)-ep*max(x,z)+1) + exp(2*ep*(x+z)).*(ep*x+ep*z-1));
    ...
    % GF: in the book in comments.
%     s = -exp(-ep*(x+z))/(4*ep^3*(exp(2*L*ep)-1)^2) .* ...
%         (   exp(2*L*ep)*(2*L*ep-ep*(x+z)-1) + ...
%             exp(4*L*ep)*(ep*(x+z)+1) + ...
%             exp(2*ep*(L+x+z)).*(2*L*ep-ep*(x+z)+1) + ...
%             exp(2*ep*(x+z)).*(ep*(x+z)-1) + ...
%             exp(2*ep*(min(x,z)+2*L)).*(-ep*abs(x-z)-1) + ...
