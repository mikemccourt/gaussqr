...
if aux_plots
    h_mle = figure;
    mlevec = arrayfun(@(ep)StatFit2_param_func(1,ep,DM,rbf,y),epvec);
    semilogx(epvec,mlevec)
end
ep_MLE = fminbnd(@(ep)StatFit2_param_func(1,ep,DM,rbf,y),.1,10);

% Find the MLE process variance, if needed 
if aux_plots
    h_cv = figure;
    pvvec = arrayfun(@(ep)StatFit2_param_func(2,ep,DM,rbf,y),epvec);
    semilogx(epvec,pvvec)
end
ep_PV = fminbnd(@(ep)StatFit2_param_func(2,ep,DM,rbf,y),.1,10);

% Find the LOOCV ep
if aux_plots
    h_cv = figure;
    cvvec = arrayfun(@(ep)StatFit2_param_func(3,ep,DM,rbf,y),epvec);
    semilogx(epvec,cvvec)
end
ep_CV = fminbnd(@(ep)StatFit2_param_func(3,ep,DM,rbf,y),.1,10);

% Find the average kriging variance at the eval points
if aux_plots
    h_cv = figure;
    kvvec = arrayfun(@(ep)StatFit2_param_func(4,ep,{DM,DM_eval},rbf,y),epvec);
    plotyy(epvec,kvvec,epvec,pvvec.*kvvec,@semilogx,@semilogx);
end
ep_KV = fminbnd(@(ep)StatFit2_param_func(4,ep,{DM,DM_eval},rbf,y),.1,10);
