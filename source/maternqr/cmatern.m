...
elseif beta==2 && Mfix==0
    ...
    % GF: This formula seems incorrect 
    ...
    % GF: This formula seems correct, but unstable for large ep (taken from
    % Casey and Will's Mathematica nb derivation-of-green's-kernel-for-beta=2,epsilon-geq-0
    %s = (4*ep*minvals.*sinh(ep*L).*cosh(ep*minvals).*...
    %    ( sinh(2*ep*L)+cosh(2*ep*L)).*sinh(ep*(L-maxvals)) + ...
    %      sinh(ep*minvals).*(cosh(ep*maxvals)-sinh(ep*maxvals)) .* ...
    %      ((-2*ep*L+ep*maxvals+1).*(sinh(2*ep*L)+cosh(2*ep*L)) - ...
    %      (ep*maxvals+1).*(sinh(4*ep*L)+cosh(4*ep*L)) + ...
    %      (2*ep*L-ep*maxvals+1).*(sinh(2*ep*(L+maxvals)) + ...
    %      cosh(2*ep*(L+maxvals))) + (ep*maxvals-1).*(sinh(2*ep*maxvals)+cosh(2*ep*maxvals)))) ./ ...
    %      (2*ep^3*(sinh(2*ep*L)+cosh(2*ep*L)-1).^2);
    % GF: This one seems to be almost correct and stable
    % GF: It looks like this is differs from the series by some constant multiple
%     s = (1/(4*ep^3*(exp(2*ep*L) - 1)^2))*exp(ep*(-(x + z))) .* ...
%         (exp(2*ep*L)*(2*ep*L-ep*x-ep*z-1) + exp(2*ep*(L+x+z)).*(2*ep*L-ep*x-ep*z+1) + ...
%          exp(2*ep*(2*L+min(x,z))).*(ep*min(x,z)-ep*max(x,z)-1) - ...
%          exp(2*ep*(L+max(x,z))).*(2*ep*L+ep*min(x,z)-ep*max(x,z)+1) + ...
%          exp(2*ep*(L+min(x,z))).*(-2*ep*L-ep*min(x,z)+ep*max(x,z)+1) + exp(4*ep*L).*(ep*x+ep*z+1) + ...
%          exp(2*ep*max(x,z)).*(ep*min(x,z)-ep*max(x,z)+1) + exp(2*ep*(x+z)).*(ep*x+ep*z-1));
%     % GF: I verified the following version with Mathematica and also put it
    % GF: in the book.
    s = -exp(-ep*(x+z))/(4*ep^3*(exp(2*ep*L)-1)^2) .* ...
        (   exp(2*ep*L)*(2*ep*L-ep*(x+z)-1) + ...
            exp(4*ep*L)*(ep*(x+z)+1) + ... 
            exp(2*ep*(L+x+z)).*(2*ep*L-ep*(x+z)+1) + ... 
            exp(2*ep*(x+z))*(ep*(x+z)-1) + ... 
            exp(2*ep*(2*L+min(x,z))).*(-ep*abs(x-z)-1) + ...
            exp(2*ep*max(x,z)).*(-ep*abs(x-z)+1) + ...
            (exp(2*ep*(L+max(x,z)))+exp(2*ep*(L+min(x,z))))*(1-2*ep*L+ep*abs(x-z)));
else
