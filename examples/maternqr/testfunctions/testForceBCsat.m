...
i = 0; % 2*i is the order of the derivative to examine
lbsat = 5;
rbsat = 5;
lbound = 0;
rbound = 1;
...
rbval = 1e10;
range = [lbound,rbound];
test = 0; % 0 means test add, 1 means test mult

if test == 0 % test additive version
    g = forceBCsatADD( f, lbsat, rbsat, lbound, rbound, lbval, rbval )
    g = g / 10^(lbsat+rbsat) % normalization factor
    d = symfun(diff(g,2*i),x);
    figure;
    ezplot(d,range);
    disp(['L boundary value: ',num2str(eval(d(lbound)))]);
    disp(['R boundary value: ',num2str(eval(d(rbound)))]);
