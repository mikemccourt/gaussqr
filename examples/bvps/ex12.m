...
for t=dt:dt:T
    ...
    intres = ex12_resBC(GQRtrue.coef,GQR,x,uold,dt,BC,t);
    ...
    GQRlin = gqr_solveprep(1,x,ep,alpha);
    phi = gqr_phi(GQRlin,x);
    phixx = gqr_phi(GQRlin,x,2);
    A = phi/dt - phixx;
    A([1,end],:) = phi([1,end],:)/dt;

    % Store the diffusivity coefficients for use in the residual
    GQRlin.DIFF_kk = DIFF_kk;GQRlin.DIFF_z = DIFF_z;
    GQRlin.DIFF_C = DIFF_C;GQRlin.DIFF_k0 = DIFF_k0;
...
%     S_u_t = -exp(-t)*(1-x.^2);
    ...
    linres = ex12_resBC(GQRlin.coef,GQRlin,x,uold,dt,BC,t);
...
%     c = GQRtrue.coef;
    newcoef = lsqnonlin(@(coef) ex12_resBC(coef,GQR,x,uold,dt,BC,t),c,[],[],opts);
    GQR.coef = newcoef;
    
    ur = gqr_eval(GQR,x);
    errnln = errcompute(ur,utrue);
    nlnres = ex12_resBC(newcoef,GQR,x,uold,dt,BC,t);
