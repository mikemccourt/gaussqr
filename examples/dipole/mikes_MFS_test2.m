...
Nvec = 100:50:1200;                % Number of desired interior points
% Fraction of collocation points used as MFS centers
MFS_frac = 1.0; % Should be (0.0,1.0], probably 1.0
% Location of points for the centers
ctr_sphere_factor = 1.3; % Should be greater (but not much greater) than 1
...
[fund_sol, dx_fs, dy_fs, dz_fs] = pickRBF('fundamental_3d');
...
% The first point is a reference point for potential ("reference electrode")
% and it is used to evaluate potential differences
delta_phi_an = phi_an - phi_an(1);
...
for Npnts = Nvec
    ...
    [POINTS, NORMALS] = BallGeometry(R, Npnts, 'mfs');
    bdydata = POINTS.bdy11;
    N_bdy = size(bdydata,1);
    
    % Centers outside the domain
    ctrs = SphereSurfGoldPoints(floor(MFS_frac*Npnts), ctr_sphere_factor*R);
    ...
    A = bsxfun(@times,normvecs(:,1),dx_fs(DM_bdydata_neu,dx_bdydata_neu));
    B = bsxfun(@times,normvecs(:,2),dy_fs(DM_bdydata_neu,dy_bdydata_neu));
    C = bsxfun(@times,normvecs(:,3),dz_fs(DM_bdydata_neu,dz_bdydata_neu));
    BCM_neu = A + B + C;
    ...
    BCM_dir = fund_sol(DM_bdydata_dir);
    ...
    %--------------------------------------------------------------------------
    % Potential
    errvec(k) = errcompute(delta_phi,delta_phi_an);
